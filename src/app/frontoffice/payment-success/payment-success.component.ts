  ...
  template: `
    <div class="result-page">
      <div class="result-card" *ngIf="!isVerifying">
        ...
        <p>Your Flouci payment has been confirmed.</p>
        <p class="ref" *ngIf="ref">Reference: <strong>{{ ref }}</strong></p>
        <p class="plan-info" *ngIf="planName">Plan: <strong>{{ planName }}</strong> — {{ amount }} TND</p>
      ...
      <div class="result-card" *ngIf="isVerifying">
        ...
        <p>Verifying your Flouci payment...</p>
      </div>
      <div class="result-card error" *ngIf="verificationFailed">
        <div class="icon">⚠️</div>
        <h2>Verification Issue</h2>
        <p>We couldn't verify your payment automatically, but don't worry — our admin team will check it manually.</p>
        <button class="btn-primary" (click)="goHome()">Go to My Subscription</button>
      </div>
    </div>
  `,
  styles: [`
    .result-page { display:flex; justify-content:center; align-items:center; min-height:100vh; background:#0f1117; }
    ...
    .plan-info { font-size:0.9rem; color:#d1d5db; }
