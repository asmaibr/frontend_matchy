...
export class TranslationService {
  ...
  translateContent(contentId: number, targetLanguage: string): Observable<ContentTranslation> {
    return this.http.post<ContentTranslation>(`${this.apiUrl}/translate/${contentId}/${targetLanguage}`, {});
  }
  ...
  getSupportedLanguages(): Language[] { return this.languages; }
  getLanguageName(code: string): string { return this.languages.find(l => l.code === code)?.name ?? code; }
  getLanguageFlag(code: string): string { return this.languages.find(l => l.code === code)?.flag ?? '🌐'; }
}
